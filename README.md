...
| Paths and tracks | `#6a6a6a`, dashed, thin | broken lines — a way through, not a street |
| Minor roads → secondary roads | `#5a5a5a` → `#9a9a9a` | mid dots |
...
| The world, today | What is happening, on the globe, from far out only, with a guess at whether the day is going well — a word list read in the page, because no free sentiment service will talk to a browser: the day's stories with a line and a few words over the place, and every earthquake over M4.5. Refreshed every ten minutes, kept for 24 hours in your own browser | Wikipedia Current events (CC BY-SA 4.0) + USGS (public domain) |
| How much of the world | Fourteen days of Wikipedia's Current events, not two, and a week of quakes, not a day: **330 stories across 103 countries** on a fresh load (90 quakes, 227 placed by name, 13 from the desk). A day older than yesterday is final on Wikipedia, so it is read once and remembered — a later look costs four requests. The list is one story per country first, newest each, then the rest: the first 100 of the list are 100 different countries. `NEWS ON THE GLOBE` goes to 195 now and starts there. 195 countries with a story each is not on offer — in two weeks the sources simply never mention about ninety of them |
| One story a country, every day | A job on GitHub's own timer (`.github/workflows/news-by-country.yml`, 05:20 UTC daily) runs `scripts/news-by-country.mjs`, which asks each of the 195 countries' own press first — 172 live feeds in 143 countries, every one probed before it went in `scripts/news-feeds.json`, AllAfrica's per-country headlines for Africa among them — and Google News searched for the country by name where the press has no open feed. The newest story from the last three days wins (a week for the search) and the outlet's name travels with it. It writes `data/news-by-country.json` and commits it; the page reads that file and nothing else. First run: **192 of 195 countries**, 124 from their own press, 68 from the search. Chad and Jordan share their names with people and got sneakers and shortstops until the rule was tightened to demonym, capital or "Chad:" — now they are honestly missing on a quiet day, as is any country with nothing in a fortnight. Of 293 feeds looked at, 197 were live and 96 dead: 47 gone, 22 blocking robots, 19 answering with a web page instead of a feed | The country's press, AllAfrica, Google News RSS |
...
| `WHO FLOATS OVER A PLACE YOU FIND` | What that figure is: a ghost, a person, a dog, a bicycle, a motorbike, a car, a van, a bus — or nobody. It holds the spot and breathes up and down. `W` `A` `S` `D` walk it around, `SPACE` jumps and holding it jumps far higher — a tap about 2.5 m, a held key about 13. `WALKING SPEED` in `SETTINGS` sets how fast: 1 to 200 m/s, starting at **30 — 108 km/h**, which crosses a street-zoom view in 3.2 seconds where the old fixed 12 took seven. Measured on open ground: 3 → 2.7, 12 → 10.6, 30 → 26.5, 60 → 53.0. Hold `SHIFT` with any direction for ten times that and half again the jump: measured 3.6 m in 300 ms walking, 36.3 m boosted, and a peak of 15.5 m against 23 m. Walking is 12 m/s and gravity 9.81 m/s², in metres rather than pixels, so zoom changes only how much you can see. The camera has slack in it: the figure walks across the middle of the screen — a box 45% out across, 35% up — and the map only starts moving when it reaches the edge of that box, by the overshoot alone. It used to be pinned to the exact centre, which meant it never appeared to move at all. Buildings carry their real heights, so a wall only stops what is shorter than it: jump onto a roof and you stand on it, walk off the edge and you fall. The map pans to keep up |
...
| `FIND A COUNTRY` | Any of the 195, by name or by typing a few letters: the map frames it, its name is decoded across the middle of it and gone in a flash, its ten biggest cities arrive one by one over five seconds and leave at ten, a light runs round its border, and the panel reads out its capital and its population |
